    ...
    public class FiefManagementVM : ViewModel
    {
        ...
        private void InitializeNativeVMs(Settlement settlement)
        {
            try
            {
                ...
                if (settlement?.Town != null)
                {
                    ...
                    FiefManagementLog.Debug("FiefManagement", $"Initialized native VMs for {settlement.Name}");
                }
            }
            catch (Exception ex)
            {
                FiefManagementLog.Debug("FiefManagement", $"Failed to initialize native VMs: {ex.Message}");
        ...
        private void ApplyBuildingChanges()
        {
            ...
            try
            {
                ...
                FiefManagementLog.Debug("FiefManagement", $"Applied building changes to {currentFief.Name}");
            }
            catch (Exception ex)
            {
                FiefManagementLog.Error("FiefManagement", "ApplyBuildingChanges", ex);
            }
        }
