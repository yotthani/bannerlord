...
namespace LayeredArmor
{
    ...
    {
        private static readonly Dictionary<Hero, Dictionary<ArmorLayer, ArmorLayerData>> _heroArmorLayers
        ...
        public static float CalculateCombinedArmor(Hero hero, EquipmentIndex slot)
        {
            ...
            var settings = LayeredArmorSettings.Get();
            if (!settings.EnableLayeredArmor)
                return baseArmor;
            ...
            var mode = settings.ArmorLayerCalculationDropdown?.SelectedIndex ?? 1;

            return mode switch
            {
                0 => baseArmor + middleArmor + innerArmor,                                    // Additive
                1 => baseArmor + (middleArmor + innerArmor) * settings.UnderArmorBonusPercent, // Highest + Bonus
