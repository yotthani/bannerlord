...
namespace EquipPresets
{
    ...
    public class EquipmentPresetCampaignBehavior : CampaignBehaviorBase
    {
        ...
        public static bool IsEnabled
        {
            get
            {
                try { return EquipPresetsSettings.Get().EnableEquipmentPresets; }
                catch { return true; }
            }
        }
        ...
        {
            SafeExec.Execute(FEATURE_NAME, "RegisterEvents", () =>
            {
        ...
        {
            SafeExec.Execute(FEATURE_NAME, "SyncData", () =>
            {
        ...
        private void OnSessionLaunched(CampaignGameStarter starter)
        {
            ...
            if (!IsEnabled)
            {
                EquipPresetsLog.Debug($"{FEATURE_NAME} is disabled in settings.");
                return;
            }

            SafeExec.Execute(FEATURE_NAME, "SessionLaunched", () =>
            {
        ...
        public List<HoNEquipmentPreset> GetPresetsForHero(Hero hero)
        {
            ...
            {
                EquipPresetsLog.Error(FEATURE_NAME, "GetPresetsForHero", ex);
            }
        ...
        {
            SafeExec.Execute(FEATURE_NAME, "AddPreset", () =>
            {
                if (!IsEnabled || hero == null || preset == null)
                    return;

                var settings = EquipPresetsSettings.Get();
                string heroId = hero.StringId;
                ...
                EquipPresetsLog.Debug($"Added preset '{preset.Name}' for {hero.Name}");
            });
        }
        ...
        {
            return SafeExec.Execute(FEATURE_NAME, "RemovePreset", () =>
            {
                if (!IsEnabled || hero == null || preset == null)
                    return false;
        ...
        {
            return SafeExec.Execute(FEATURE_NAME, "RemovePresetById", () =>
            {
        ...
        {
            SafeExec.Execute(FEATURE_NAME, "UpdatePreset", () =>
            {
                if (!IsEnabled || hero == null || preset == null)
                    return;
        ...
        public HashSet<string> GetAllPresetItemIds(Hero hero)
        {
            ...
            SafeExec.Execute(FEATURE_NAME, "GetAllPresetItemIds", () =>
            {
                if (!IsEnabled) return;
