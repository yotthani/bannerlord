...
### Late Session - Real Sprites & Animations

**Real Ring Sprites Integrated**
- All 20 rings now use actual sprites from `ui_ring_system` sprite sheet
- Sprite binding: `Sprite="@SpriteName"` in XML, mapped via `GetSpriteNameForRing()`
- Sprites: `rings\ring_one`, `rings\ring_elven_narya`, `rings\ring_dwarf_1`, `rings\ring_mortal_1`, etc.
- Aspect ratio: 384x441 (~0.87:1, taller than wide)

**Sprite Sizes (25% larger than initial)**
- Mortal: 50x58 button, 80x115 container
- Dwarven: 56x65 button, 90x130 container  
- Elven: 68x78 button, 100x145 container
- One Ring: 100x115 button, 140x190 container

**Enhanced Glow Effects**
- Table glow: 0.4 base opacity (up to 0.7 when ring close)
- Ring aura: 0.15 base opacity (up to 0.55 when floating high)
- More dramatic variation with bobbing

**Preview Ring Animation (NEW)**
- Pulsing glow: Outer/inner glow rings pulse (0.2→0.5 opacity, 1.3x phase)
- Gentle bob: Ring floats ±3px (0.8x phase speed)
- Real sprite: Uses `@PreviewRingSprite` bound to equipped ring
- Properties: `PreviewPulseScale`, `PreviewGlowOpacity`, `PreviewBobOffset`, `PreviewRingSprite`
- Updated in `UpdatePreviewAnimation()` called from `UpdateAnimations()`

## 2024-12-30: New Features Implementation

### 5 New Features Scaffolded & Implemented

#### Feature 1: Siege Auto-Dismount
**Files:** `Features/SiegeDismount/`
- `SiegeDismountBehavior.cs` - Mission behavior that detects siege missions and handles mount state
...
**Files:** `Features/LayeredArmor/`
- `LayeredArmorManager.cs` - Manages armor layers per hero (Inner/Middle/Outer)
- `LayeredArmorPatches.cs` - Patches `Agent.GetBaseArmorEffectivenessForBodyPart`
...
**Files:** `Features/MixedFormations/`
- `MixedFormationLayoutBehavior.cs` - Mission behavior for formation management
- `FormationLayoutPatches.cs` - Patches formation positioning

**Layout Options:**
- Infantry Front, Ranged Back
- Ranged Front, Infantry Back
- Ranged Wings, Infantry Center
- Checkerboard

#### Feature 4: Battle Action Bar
**Files:** `Features/BattleActionBar/`
- `BattleActionBarVM.cs` - ViewModel with context-sensitive actions
- `BattleActionBarMissionView.cs` - Mission view that shows/hides bar
- `TroopStanceManager.cs` - Manages locked stances (brace, pike wall, etc.)

**Actions by Unit Type:**
- Ranged: Hold Fire, Free Fire, Volley Fire
- Polearm: Brace for Cavalry, Pike Wall
- Shield: Shield Wall, Testudo
- Cavalry: Line Charge, Skirmish

**UI:** `HeirOfNumenorGUI/GUI/Prefabs/BattleActionBar.xml`

#### Feature 5: Smart Cavalry AI
**Files:** `Features/SmartCavalryAI/`
- `SmartCavalryAIBehavior.cs` - Mission behavior for cavalry state management
- `CavalryChargeController.cs` - Coordinated charge logic

**Cavalry States:**
- Idle → Forming → Charging → PassingThrough → Reforming

**Features:**
- Line formation before charge
- Synchronized charge timing
- Friendly collision avoidance
- Auto-reform after pass-through

### Settings Added (Settings.cs)
- 5 new setting groups with ~20 total options
- All enums defined: `SiegeMountBehavior`, `ArmorLayerCalculationMode`, `MixedFormationLayout`, `ActionBarPosition`

### SubModule.cs Updated
- Added using statements for new features
- Added `OnMissionBehaviorInitialize` to register mission behaviors conditionally

### Localization Added
- 20+ new strings for all features in `std_module_strings_xml.xml`
