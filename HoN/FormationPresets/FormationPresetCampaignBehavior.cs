...
namespace FormationPresets
{
    ...
    public class FormationPresetCampaignBehavior : CampaignBehaviorBase
    {
        ...
        public static bool IsEnabled
        {
            get
            {
                try { return FormationPresetsSettings.Get().EnableFormationPresets; }
                catch { return true; }
            }
        }
        ...
        private void OnSessionLaunched(CampaignGameStarter starter)
        {
            ...
            if (!IsEnabled)
            {
                FormationPresetsLog.Debug($"{FEATURE_NAME} is disabled in settings.");
                return;
            }
        ...
        {
            SafeExec.Execute(FEATURE_NAME, "MarkDirty", () =>
            {
                _savedPresets = FormationPresetManager.GetPresetsForSaving();
            });
        }
        ...
        public bool IsAutoAssignEnabled()
        {
            try
            {
                return IsEnabled && FormationPresetsSettings.Get().AutoAssignHeroes;
            }
            catch
            {
                return true;
        ...
        public int GetMaxPresets()
        {
            try
            {
                return FormationPresetsSettings.Get().MaxFormationPresets;
            }
            catch
            {
                return 10;
