...
{
    internal static class FormationPresetsLog
    {
        public static void Debug(string message)
        {
            try
            {
                if (FormationPresetsSettings.Get().DebugMode)
                    InformationManager.DisplayMessage(new InformationMessage($"[FormationPresets] {message}", Colors.Gray));
            }
            catch { }
        }
    ...
    internal static class SafeExec
    {
        public static bool Execute(string feature, string op, Action action)
        {
            ...
            catch (Exception ex) { FormationPresetsLog.Error(feature, op, ex); return false; }
        }
        ...
        public static T Execute<T>(string feature, string op, Func<T> func, T defaultValue = default)
        {
            ...
            catch (Exception ex) { FormationPresetsLog.Error(feature, op, ex); return defaultValue; }
        }
    }
}
