...
### v2.6.2 - Complete Classification System Overhaul
- **FIX:** FaceLength now uses ratios[2] (was duplicate of FaceWidth!)
- **ADDED:** EyeSize, EyeSpacing, EyeShape, CheekFullness, CheekboneProminence
- **IMPROVED:** FaceShape uses lowerThird, JawShape uses jawDrop, LipFullness combines two signals
- **REMOVED:** NoseTip (unreliable)
- **CALIBRATED:** All thresholds adjusted for better distribution
- **DEBUG:** Logs ALL ratio values every 50 faces
- ⚠️ DELETE hierarchical_knowledge.dat - old data incompatible!

### v2.6.2 - CRITICAL: Classification Ratios Fixed
- **BUG:** Was using JAW ratios (0.5) instead of actual feature ratios
  - NoseWidth used ratios[3] (jaw length) → NOW ratios[30] (nostril width)
  - MouthWidth used ratios[4] (jaw length) → NOW ratios[34] (mouth width)
  - LipFullness used ratios[7] (jaw segment) → NOW ratios[38] (lip thickness)
- **ADDED:** NoseLength classification using ratios[33]
- **ADDED:** Debug logging every 50 faces
- **IMPACT:** Old hierarchical_knowledge.dat was trained wrong - consider deleting

### v2.4.0 - FaceShape Feature Added
- NEW: FaceShape feature (Round, Oval, Angular)
- Detection via: heightRatio, segmentVariance, cheekToChinRatio
- Enables learning different morphs for round vs oval vs angular faces
- High priority in FeaturePriority (after FaceWidth)
